...
class GeneratorCore(object):
    def __init__(self, disassembly, branches):
        ...
        # Keep track of how many levels deep for linked branches
        self.depth = 0
        ...
        if instr in BRANCH_INSTRUCTIONS:
            if 'bl' in instr:
                ...
                if op in REGISTER_NAMES:
                    ...
                    self.subroutine_stack.append(index-int(IMAGEBASE,16))  
            ...
            if 'bl' in instr:
                ...
                if op in REGISTER_NAMES:
                    ...
                    self.subroutine_stack.append(index-int(IMAGEBASE,16))  
                    dest = self.register_branches[op].pop()
                    #self.subroutine_stack.append(dest-int(IMAGEBASE,16))  INCORRECT
    ...
    def link_subroutines(self, bl_addrs, mem_instr):
        for i in bl_addrs:
            ...
            op = mem_instr[i].op
            if op in REGISTER_NAMES:
                while self.register_branch_calc(mem_instr[index].instr, mem_instr[index].op, index) == False:
                    index -= 1
                    while mem_instr[index] == 0:
                        index -= 1

    # Link all other instructions
    def instruction_linking(self, start, mem_instr):
        ...
        while i < len(mem_instr)-1:
            try:
                ...
                if i in self.link_branch_instructions:
                    i += 1
    ...
    def generate_paths(self):
        ...
        #        instr = MEM_INSTR[i].instr
        #        op = MEM_INSTR[i].op
        #        if 'ldr' in instr and 'pc' in op and 'r8' in op:
        #            print hex(i+int(IMAGEBASE,16)), instr, op
        #    i += 1

        self.link_detection(MEM_INSTR, STARTING_ADDRESS)
        self.instruction_linking(i, MEM_INSTR)
        addr = int('3c16',16)
        print(MEM_INSTR[addr].instr, MEM_INSTR[addr].op)
        for i in self.paths:
            g = i - int(IMAGEBASE,16)
            instrtmp = MEM_INSTR[g].instr
            optmp = MEM_INSTR[g].op
            if instrtmp in BRANCH_INSTRUCTIONS or instrtmp in CONDITIONAL_BRANCHES:
                if optmp in REGISTER_NAMES:
                    print 'penis'
                    s = instrtmp + ' ' + optmp
                    print("Address: %s  First Path: %s   Branch Path: %s   instruction: %s"%(hex(i), hex(self.paths[i].path), hex(self.paths[i].branch_path), s))
        #tmp = False
        #for i in self.paths:
        #    if self.paths[i].path == addr or self.paths[i].branch_path == addr:
        #        tmp = True
        #print tmp
