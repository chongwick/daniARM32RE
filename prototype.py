    ...
    def instruction_linking(self, i):
        ...
        while True:
            try:
                ...
                tmp = self.branch_end_handler()
                if tmp == 0:
                    return
                else:
                    i = tmp

            if i+int(IMAGEBASE,16) in self.paths and self.return_from_sub == False:
                tmp = self.branch_end_handler()
                if tmp == 0:
                    return
                else:
                ...
                if (instr == 'b' and op not in REGISTER_NAMES) and (int(op.split('#')[1],16)-int(IMAGEBASE,16) == i) and not self.return_from_sub:
...
def get_path(target):
    ...
    while curr != target:
        ...
        if curr not in target_map:
            ...
            if PATHS[curr].path != 0:
                if PATHS[curr].branch_path != 0:
                   untaken_branches.append(PATHS[curr].branch_path) 
                curr = PATHS[curr].path
            else:
                curr = PATHS[curr].branch_path
        else:
